...
export function ProcessContractEntry() {
  ...
  const handleItemChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === "finishedFabricCodeId") {
      const codeObj = finishFabricList.find((cat: any) => cat.id?.toString() === value);
      update.finishedFabricName = codeObj?.finishName || "";
    }
    if (name === "greigeFabricCodeId") {
      const codeObj = fabricMasterList.find((cat: any) => cat.id?.toString() === value);
      update.greigeFabricName = codeObj?.fabricName || "";
    }
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      reset();
      setDyeingDtl([]);
      navigate("/transaction/process-contract-details", { state: { tab: "processContactFabric" } });
    } catch (err) {
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-4 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-secondary-700">Finished Fabric Code <span className="text-red-600">*</span></label>
                    <select
                    ...
                    >
                      <option value="">Select Finished Fabric Code</option>
                      {finishFabricList?.map((ff: any) => (
                        <option key={ff.id} value={ff.id}>{ff.finishCode}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-secondary-700">Greige Fabric Code <span className="text-red-600">*</span></label>
                    <select
                      className="input mt-1"
                    ...
                    >
                      ...
                      {fabricMasterList?.map((fm: any) => (
                        <option key={fm.id} value={fm.id}>{fm.fabricCode}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                ...
                <table className="w-full border-collapse border border-gray-300 text-center">
                  <thead className="bg-gray-100">
                    <tr>
                      ...
                      <th className="border p-2">Finished Fabric Name</th>
                      <th className="border p-2">Greige Fabric Code</th>
                      <th className="border p-2">Greige Fabric Name</th>
